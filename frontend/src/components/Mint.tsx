...
const Mint = () => {
  ...
  const [assets, setAssets] = useState<backend.AssetInfo[]>([]);
  const [addresses, setAddresses] = useState<Address[]>([]);
  const [modalType, setModalType] = useState('');
  const [selectedAsset, setSelectedAsset] = useState<backend.AssetInfo | null>(
    null,
  );
  ...
  const handleCreateAsset = async (values: backend.AssetInfo) => {
    ...
    try {
      ...
      timeTaken = // TODO to rewrite
        timeTaken < 1
          ? 'less than a second'
          : `${timeTaken.toFixed(1)} seconds`;
      message.success({
